<template>
    <div class="divComGrpCodList">
        <NoticeModal v-if="modalState.modalState" :noticeSeq="noticeSeq" @modalClose="()=>{noticeSeq = undefined}" @postSuccess="searchList"></NoticeModal>
            ...
            <tbody>
                <template v-if="noticeList">
                    <template v-if="noticeList.listCount > 0">
                        <tr v-for="item in noticeList.noticeList" :key="item.noti_seq" @click="handlerDetail(item.noti_seq)">
                            <td>{{ item.noti_seq }}</td>
